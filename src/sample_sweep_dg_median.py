...
with torch.no_grad():
    for dg in dg_bins:
        ...
        for g in decoded:
            ...
            error_c = abs(real_dg - mfe_c)
            mutation_c = mutation_count(g, target_str)

            decoded_seqs.append(g)
            hairpins.append(hairpin)
            mfe_values.append(mfe_c)
            error_values.append(error_c)
            mutation_values.append(mutation_c)

        # Median-error sample selection
        errors_tensor = torch.tensor(error_values)
        median_idx = torch.argsort(errors_tensor)[len(errors_tensor) // 2]

        g = decoded_seqs[median_idx]
        hairpin = hairpins[median_idx]
        mfe = mfe_values[median_idx]
        error = error_values[median_idx]
        mutations = mutation_values[median_idx]
