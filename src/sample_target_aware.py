...
# ==== Initialize model ====
unet = Unet1D(dim=64, seq_len=seq_len, channels=4, cond_dim=1, target_dim=4)
...
with torch.no_grad():
    ...
    # Placeholder: load or generate a target_tensor matching shape [num_samples, seq_len, 4]
target_tensor = torch.load("../data/processed/target_onehot.pt")[:num_samples].to(device)
samples = diff_model.sample((num_samples, seq_len, 4), cond_tensor, target_tensor, device)
